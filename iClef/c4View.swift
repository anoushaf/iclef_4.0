...
struct c4View: View {
    ...
    var l = getLan()
    let answers = [ "G", "A","B", "C", "D", "E", "F"]
    @State var timeRemaining = getTime()
    var body: some View {
        ZStack{
            Image("background3").resizable().ignoresSafeArea(.all)
            VStack(alignment: .center){
                ...
                Image("alto4-" + String(self.cardNum)).resizable().aspectRatio(contentMode: .fit)
                HStack{
                    Spacer()
                    Button(action: {
                        if(self.result == -1){
...
struct c4View_Previews: PreviewProvider {
