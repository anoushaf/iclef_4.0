...
struct c3View: View {
    ...
    var l = getLan()
    let answers = ["B", "C", "D", "E", "F", "G","A"]
    @State var timeRemaining = getTime()
    var body: some View {
        ZStack{
            Image("background4").resizable().ignoresSafeArea(.all)
            VStack(alignment: .center){
                HStack(){
                    if(getTime() > 0){
                        ...
                        Text("     ").foregroundColor(.black).font(.system(size: 34, design: .rounded))            .padding(.leading, 20.0)
                ...
                Image("alto3-" + String(self.cardNum)).resizable().aspectRatio(contentMode: .fit)
                HStack{
                    Spacer()
                    Button(action: {
                        if(self.result == -1){
...
struct c3View_Previews: PreviewProvider {
