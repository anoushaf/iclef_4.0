...
struct gView: View {
    ...
    var body: some View {
        ZStack{
            Image("background1").resizable().ignoresSafeArea(.all)
            VStack(alignment: .center){
                HStack(){
                    ...
                    if(self.result == 2 || (self.t >= getTime() && getTime() > -1)){
                        Text(answers[self.index]).foregroundColor(Color(red:0.6, green: 0, blue: 0)).font(.system(size: 45, design: .rounded ))
                    }
                ...
                Image("card" + String(self.cardNum)).resizable().aspectRatio(contentMode: .fit)
                //Spacer()
