...
struct c1View: View {
    ...
    var l = getLan()
    let answers = ["F", "G","A", "B", "C", "D", "E"]
    @State var timeRemaining = getTime()
    var body: some View {
        ZStack{
            Image("background6").resizable().ignoresSafeArea(.all)
            VStack(){
                HStack(){
                    if(getTime() > 0){
                ...
                Image("alto" + String(self.cardNum)).resizable().aspectRatio(contentMode: .fit)
                //Spacer()
...
struct c1View_Previews: PreviewProvider {
